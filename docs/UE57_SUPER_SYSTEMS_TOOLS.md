...
- AI authoring and navigation scaffolds:
    `create_behavior_tree`, `create_blackboard`, `add_bt_task`, `add_bt_decorator`, `configure_ai_perception`, `create_navmesh_bounds`
- Base audio authoring and playback:
    `create_metasound`, `play_sound_2d`, `play_sound_at_location`, `stop_all_sounds`
...
- Historical helpers `set_foliage_properties`, `list_foliage_types`, and `remove_foliage` are de-registered and should not be advertised as current callable tools.
