...
| Set shading model | `set_material_shading_model` | Use "Subsurface" for foliage (closest to TwoSidedFoliage) |
...
| Inspect reusable foliage assets | `list_foliage_types` | Check existing foliage definitions before painting or simulation |
| Clear foliage | `remove_foliage` | Remove instances from an area, optionally filtered by foliage type |
...
27. **wire_blueprint_from_description** — JSON → real K2 nodes wired in Blueprint event graph.
28. **compose_cinematic_sequence** — JSON shots → playable ULevelSequence with cameras and cuts.
29. **compose_pcg_graph** — JSON → UPCGGraph with typed nodes and wired connections.
...
| Task | Tool | Notes |
|---|---|---|
| Wire event graph | `wire_blueprint_from_description` | JSON nodes/connections → real K2 nodes. Supports: event, call, variable_get/set, branch, custom_event |
...
| Task | Tool | Notes |
|---|---|---|
| Create cinematic | `compose_cinematic_sequence` | JSON shots → ULevelSequence with camera tracks, keyframes, cuts |
...
| Task | Tool | Notes |
|---|---|---|
| Build PCG graph | `compose_pcg_graph` | JSON nodes/edges → UPCGGraph. Attach to PCG Volume to execute. |
