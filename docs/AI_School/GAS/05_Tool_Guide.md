...
| Task | Tool | Notes |
|---|---|---|
| Verify runtime behavior | `run_quick_playtest` | Current default public runtime-proof lane for GAS behavior and feel |
...
- [ ] asset surface verified (`get_gas_assets`)
- [ ] runtime behavior verified in play (`run_quick_playtest`)
...
`add_gameplay_tag` -> `create_gameplay_effect` -> `configure_gameplay_effect` -> `create_gameplay_ability` -> `set_ability_cost` -> `set_ability_cooldown` -> `set_ability_policies` -> `assert_blueprint_compiles` -> `get_gas_assets` -> `add_ability_to_actor` -> `run_quick_playtest`
...
`add_gameplay_tag` -> `create_gameplay_effect` -> `configure_gameplay_effect` -> `configure_ge_stacking` -> `assert_blueprint_compiles` -> `get_gas_assets` -> `add_ability_to_actor` or effect application path -> `run_quick_playtest`
...
`create_attribute_set` -> `assert_blueprint_compiles` -> `get_gas_assets` -> actor integration work -> `run_quick_playtest`

Use for: health/mana/stamina systems, hunger/thirst meters, crafting resource pools.

### Effect Tuning Pass

`get_gas_assets` -> `configure_gameplay_effect` -> `configure_ge_stacking` -> `assert_blueprint_compiles` -> `run_quick_playtest`
