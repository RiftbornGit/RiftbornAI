...
In the shipped beta boundary, the core ASC rules are:
...
For the current default public verification lane, use:

- `get_gas_assets`
- `run_quick_playtest`
- `audit_net_replication` and `inspect_actor_replication` when network truth matters

Actor-level GAS runtime getters are not on the current default public surface. Verify exact callable tool names with `describe_tool` before depending on deeper GAS runtime introspection.
