...
|------|---------|-------------|
| `create_metasound` | Create a playable MetaSound asset | Current default public entrypoint for world-playable procedural or hybrid sources. Use for ambient loops, one-shots, gameplay cues. |
| `create_metasound_source` | Create a playable MetaSound source asset | Specialized source-authoring lane when you explicitly need the source variant scaffold. |
...
| `play_sound_2d` | Trigger a simple non-spatial playback path | Useful for UI, menu, or flat reference playback during proof. |
| `play_sound_at_location` | Trigger one-shot playback at a world location | Useful for direct in-world cue proof before longer-lived emitter setup. |
| `stop_all_sounds` | Halt current playback quickly | Useful when resetting the audio scene between proofs. |
...
Main tools: `create_metasound`, `create_metasound_source`, `create_metasound_preset`, `create_soundscape_metasound_source`, `list_metasounds`, `get_metasound_info`, `list_metasound_node_types`, `add_metasound_node`, `connect_metasound_nodes`, `set_metasound_input`, `play_metasound_preview`
...
Main tools: `audio_spatial`, `spawn_audio_component`, `set_audio_component`, `analyze_acoustics`, `play_sound_2d`, `play_sound_at_location`, `stop_all_sounds`, `run_quick_playtest`
...
`create_metasound` or `create_soundscape_metasound_source` → `set_metasound_input` → `play_metasound_preview` → `audio_spatial` → `spawn_audio_component` → `set_audio_component` → `run_quick_playtest`
...
`import_audio` → `create_metasound` → `play_metasound_preview` → `create_metasound_preset` for variants → `audio_spatial` → `spawn_audio_component`
