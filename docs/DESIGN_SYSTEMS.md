...
Design Systems are domain-specific knowledge engines that encode professional game design expertise. Each system understands one domain deeply — enough to translate natural language into concrete UE assets.

---

## 1. Overview

There are **15 Design Systems** and **5 Knowledge Bases**:

### Design Systems

| System | File | Domain |
|--------|------|--------|
| `GameDesignSystem` | `GameDesignSystem.h` (764 lines) | Full game planning — genre, loops, progression, scope, GDD |
| `LevelDesignSystem` | `LevelDesignSystem.h` (366 lines) | Rooms, lighting, PCG, layouts, environment themes |
| `VFXDesignSystem` | `VFXDesignSystem.h` (649 lines) | Niagara VFX — styles, categories, particles, emitter layers |
| `GASDesignSystem` | `GASDesignSystem.h` (539 lines) | Gameplay Ability System — abilities, effects, attributes, combos |
...
22 content categories tracked: Characters, NPCs, Enemies, Bosses, Levels, Zones, Quests, Items, Weapons, Armor, Abilities, Skills, Vehicles, Mounts, Collectibles, Achievements, Cutscenes, Dialogues, Tutorials, UIScreens, AudioTracks, VFXEffects
