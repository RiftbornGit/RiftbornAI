...
## HIGH-RISK FALLBACK: execute_python

The `execute_python` tool runs arbitrary Python code inside the Unreal Editor process via `import unreal`.
Use it only when no dedicated governed tool exists for the task and only when the
mutation/risk tradeoff is explicit.
It is not the default path for routine authoring work, and it must not be used to
bypass safer dedicated tools, confirmation gates, or public-surface constraints.
It gives you access to broad Unreal Editor APIs including materials, widgets,
Niagara, Sequencer, landscape, and PCG, so mistakes are correspondingly expensive.
