...
const LEVEL_TRANSITION_TOOL_NAMES = new Set([
    "create_level",
    "load_level",
...
const RESULT_EXTRACTORS = {
    ...
    take_screenshot: [
        { field: /Screenshot requested:\s*(.+?\.png)/, contextKey: "last_screenshot_path" },
    ],
};
...
const CONTEXT_TO_PARAMS = {
    ...
    last_screenshot_path: [
        {
            tools: ["vision_compare"],
            param: "before_path",
        },
        {
            tools: ["analyze_scene_screenshot"],
            param: "image_path",
        },
    ],
};
...
function isSafeScreenshotPath(value) {
    ...
    if (!/\.(?:png|jpg|jpeg)$/i.test(normalized)) {
        return false;
    }
    if (/^\/Saved\/Screenshots\/[A-Za-z0-9_\-./ ]{1,512}$/i.test(normalized)) {
        return true;
    }
    if (/^Saved\/Screenshots\/[A-Za-z0-9_\-./ ]{1,512}$/i.test(normalized)) {
        return true;
    ...
    if (/^(?:[A-Za-z]:\/|\/)/.test(normalized)) {
        return normalized.includes("/Saved/Screenshots/");
    }
    return false;
}
export class ContextPropagator {
    ...
    inject(toolName, params) {
        ...
        const SAFE_ASSET_PATH = /^\/Game\/[A-Za-z0-9_\-./]{1,512}$/;
        for (const [contextKey, injections] of Object.entries(CONTEXT_TO_PARAMS)) {
            ...
            const isScreenshotPath = contextKey.includes("screenshot_path");
            const isPath = contextKey.includes("path") || contextKey.includes("asset");
            const isValid = isScreenshotPath
                ? isSafeScreenshotPath(value)
                : isPath
