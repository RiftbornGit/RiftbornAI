 ...
 */
import { getDefaultProductionReadinessTier, getProductionToolNameSet } from "./surface-manifest.js";
...
const DEFAULT_PRODUCTION_READINESS_TIER = getDefaultProductionReadinessTier();
// ─── Explicit Overrides ───────────────────────────────────────────────────────
// Tools listed here have a manually-assigned tier override. Production-surface
// tools project to DEFAULT_PRODUCTION_READINESS_TIER from the manifest, so this
// map should stay focused on non-default exceptions rather than mirroring
// `production_tools`.
const EXPLICIT_TIER = {
    ...
    if (PRODUCTION_TOOL_NAMES.has(toolName)) {
        if (inferredTier === "STUB") {
            ...
            if (generatedToolNames?.has(toolName)) {
                return { tier: DEFAULT_PRODUCTION_READINESS_TIER };
            }
            return {
                tier: "STUB",
