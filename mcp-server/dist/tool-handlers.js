...
async function resolveImageSource(source, fieldName) {
    ...
    // SECURITY: Reject path traversal sequences and null bytes in local paths
    const resolved = path.resolve(trimmed);
    if (resolved.includes("\0") || trimmed.includes("..")) {
        return {
            ok: false,
            error: `${fieldName} contains invalid path characters.`,
        };
    }
    // SECURITY: Cap file size to 50 MB to prevent memory exhaustion
    const MAX_IMAGE_BYTES = 50 * 1024 * 1024;
    try {
        const stat = await fs.stat(resolved);
        if (!stat.isFile()) {
            return { ok: false, error: `${fieldName} is not a regular file.` };
        }
        if (stat.size > MAX_IMAGE_BYTES) {
            return { ok: false, error: `${fieldName} exceeds maximum image size (50 MB).` };
        }
...
async function writeTempBinaryFile(baseName, extension, bytes) {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "riftborn-"));
    const tempFilePath = path.join(tmpDir, `${safeBaseName}.${extension.replace(/^\./, "")}`);
