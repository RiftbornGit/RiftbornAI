...
class RiftScriptOrchestrator:
            ...
            try:
                ...
                if gamemode_bp_class:
                    ...
                    if character_bp_class:
                        ...
                        self.log("⚠️  Character Blueprint class not loaded", "Warning")
                else:
                    self.log("⚠️  GameMode Blueprint class not loaded", "Warning")
            except Exception as e:
    ...
    def run_phase3(self) -> tuple[bool, bool]:
        ...
        try:
            ...
            self.log("Watch for validation markers in Output Log:")
...
def cli_build(profile_name: str, spec_path) -> bool:
    ...
    if not _SAFE_NAME.match(project_name):
        orchestrator.log("Invalid project_name: must be alphanumeric/underscore/hyphen, max 128 chars", "Error")
        return False
    ...
    if '"' in project_path or "'" in project_path or '..' in project_path:
        orchestrator.log("Invalid project_path: must not contain quotes or path traversal", "Error")
        return False
