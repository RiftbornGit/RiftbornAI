...
private:
	...
	static void GenerateRooms(const TArray<TSharedPtr<FBSPNode>>& Leaves, TArray<FBSPDungeonRoom>& OutRooms, int32 MinRoomSize, int32 RoomPadding, float CellSize, FVector Origin, FRandomStream& Rng);
	static void GenerateCorridors(const TSharedPtr<FBSPNode>& Node, const TArray<FBSPDungeonRoom>& Rooms, TArray<FBSPDungeonCorridor>& OutCorridors, float CorridorWidth, float CellSize, FVector Origin);
	static FVector GetRoomCenter(const TSharedPtr<FBSPNode>& Node, const TArray<FBSPDungeonRoom>& Rooms);

	// World spawning
	static void SpawnRoomGeometry(UWorld* World, const FBSPDungeonRoom& Room, float WallHeight, const FString& FloorMesh, const FString& WallMesh, TArray<AActor*>& OutActors);
	static void SpawnCorridorGeometry(UWorld* World, const FBSPDungeonCorridor& Corridor, float WallHeight, TArray<AActor*>& OutActors);
	static void SpawnRoomLighting(UWorld* World, const FBSPDungeonRoom& Room, float WallHeight);
};
