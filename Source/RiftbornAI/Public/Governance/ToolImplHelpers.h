...
inline bool ValidateAndSanitizePath(const FString& RelativePath, FString& OutFullPath, FString& OutError)
{
	...
	// Reject Windows reserved device names (CON, PRN, NUL, AUX, COM1-9, LPT1-9)
	{
		...
		FString BaseName = FPaths::GetBaseFilename(RelativePath).ToUpper();
		if (ReservedNames.Contains(BaseName))
		{
			OutError = FString::Printf(TEXT("Path uses Windows reserved device name '%s' - security violation"), *BaseName);
			return false;
		}
	}
...
{
	const FInputSanitization::FValidationResult Validation = FInputSanitization::ValidateAssetPath(AssetPath);
