...
public:
     ...
     *
     * Defined out-of-line in SessionTaint.cpp so the proof-mode check can
     * defer to the canonical FToolContractRegistry::IsProofModeEnabled()
     * authority (secure-by-default) without forcing every includer of this
     * header to pull in ToolContract.h. Previous inline impl tainted any
     * default-secure run as "proof mode disabled" simply because the env
     * var wasn't explicitly set, polluting audit signal.
     */
