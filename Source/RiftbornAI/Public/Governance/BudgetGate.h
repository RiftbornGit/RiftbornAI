    ...
    FBudgetEvaluation EvaluateConstraint(const FBudgetConstraint& C, const FClaudeToolCall& Call) const
    {
        switch (C.Kind)
        {
            ...
            case EBudgetConstraintKind::PropertyAllowlist:
            {
                ...
                {
                    if (PropName.Equals(Allowed, ESearchCase::IgnoreCase)) return FBudgetEvaluation::Pass();
                    if (NormalizedName.Equals(Allowed, ESearchCase::IgnoreCase)) return FBudgetEvaluation::Pass();
                }
