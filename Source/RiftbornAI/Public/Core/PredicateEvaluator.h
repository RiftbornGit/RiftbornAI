...
namespace PredicateValidators
{
	...
	/** Asset does not exist at path */
	FPredicateResult AssetNotExists(const TMap<FString, FString>& Args);

	/** Editor has a current level loaded */
	FPredicateResult LevelLoaded(const TMap<FString, FString>& Args);

	/** Current editor level package is writable */
	FPredicateResult LevelNotReadonly(const TMap<FString, FString>& Args);

	/** Long package destination path is writable */
	FPredicateResult PathWritable(const TMap<FString, FString>& Args);

	/** Spawn coordinate is sane and not obviously buried */
	FPredicateResult SpawnLocationValid(const TMap<FString, FString>& Args);
