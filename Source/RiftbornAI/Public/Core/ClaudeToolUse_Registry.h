...
public:
	...
	/**
	 * Suggest tool names that are lexically close to an unrecognized name.
	 *
	 * Different from SuggestToolsForIntent: this is for "did you mean X?" on a
	 * hallucinated / typo'd tool name like `set_viewport_camera`, not a natural-
	 * language intent query. Splits on `_` so snake_case tools score on shared
	 * segments (prefix match weighted highest), and falls back to substring
	 * overlap for aliases.
	 *
	 * Only public tools are returned — internal / hidden tools are never a
	 * valid "did you mean" target for an LLM caller.
	 */
	TArray<FString> SuggestSimilarToolNames(const FString& UnknownName, int32 MaxResults = 5) const;
