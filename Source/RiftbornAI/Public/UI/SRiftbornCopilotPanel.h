...
private:
	...
	// Act Mode:      Plan → auto-confirm → PlanExecutor (immediate)
	...
	/**
	 * Model tiering: Select the best model for the request complexity.
	 * Returns the model ID to use (may differ from CurrentModelId).
	 * - Supported cloud models are Sonnet 4.6, Opus 4.6, and GPT-5.4
	 * - Local Ollama models are respected for simple work
	 * - Complex local requests can auto-upgrade to Sonnet or GPT-5.4 when available
	 */
	FString DetermineModelForComplexity(const FString& Message, int32 EstimatedSteps) const;
	 ...
	 *  anything else → "Ollama". Centralised so every call site stays in sync. */
	 ...
	 *  Anthropic model ID (e.g. `claude-code:opus-4-6` → `claude-opus-4-6`).
	...
	// Feature 11: /fix command — detect compile errors and auto-fix via LLM
	void ProcessFixCommand(const FString& Args);             // Entry point for /fix command
	void HandleBlueprintFix();                               // Detect & fix Blueprint compile errors
	void HandleCppCompileFix();                              // Detect & fix C++ compile errors via LLM
	bool IsFixCommand(const FString& Message) const;         // Detect "/fix" prefix
	
	// Feature 12: /look command — capture viewport screenshot and analyze via vision LLM
	...
	FString CaptureViewportScreenshot() const;               // Capture viewport to temp PNG, return path
