 ...
 *
 * IMMUTABLE after FromDraft(). All state is read-only behind const accessors;
 * external callers cannot mutate the frozen plan, its hash, or its steps.
 * Any edits require creating a new FPlanDraft and re-approving.
 *
 * The PlanHash is the authority on integrity — if it ever disagrees with
 * a recomputed hash of PlanJson, VerifyIntegrity() returns false and
...
public:
	...
	const FGuid& GetSourceDraftId() const { return SourceDraftId; }
	const FGuid& GetApprovalId() const { return ApprovalId; }
	FDateTime GetApprovedAt() const { return ApprovedAt; }
	const FString& GetApprovedBy() const { return ApprovedBy; }
	const FString& GetGoalLine() const { return GoalLine; }
	const TArray<FDraftToolCall>& GetSteps() const { return Steps; }
	const TArray<FString>& GetPredicates() const { return Predicates; }
	const FString& GetPlanJson() const { return PlanJson; }
	const FString& GetPlanHash() const { return PlanHash; }
	EToolRisk GetHighestRisk() const { return HighestRisk; }
	bool IsFullyReversible() const { return bFullyReversible; }
	int32 GetMutatingStepCount() const { return MutatingStepCount; }
	int32 GetDestructiveStepCount() const { return DestructiveStepCount; }
