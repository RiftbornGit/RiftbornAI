...
struct RIFTBORNAI_API FAgentTask
{
    ...
    FGuid BackingSessionId;
    bool bIsAgenticBacked = false;
...
public:
    ...
    /** Iterate over active and recent tasks (thread-safe snapshot) */
    void ForEachTask(TFunction<void(const FAgentTask&)> Callback) const
    {
        ...
        for (const auto& Pair : RecentTasks)
        {
            Callback(Pair.Value);
        }
    }
...
private:
    ...
    TArray<FGuid> RecentTaskOrder;
    static constexpr int32 MaxRecentTasks = 32;
